...
import {
    ...
    Trash2, MapPin, Star, Share2, Upload
} from 'lucide-react';
...
const ProjectList = ({ navigateTo }) => {
    ...
    const [isCreatingFromImport, setIsCreatingFromImport] = useState(false);
    const [selectorMode, setSelectorMode] = useState('CAMERA'); // 'CAMERA' or 'IMPORT'
    ...
    const handleCreateProject = async (e) => {
        ...
        } else if (isCreatingFromImport) {
            setIsCreatingFromImport(false);
            setTargetProjectId(newProject.ProjectID);
    ...
    const handleImportPhoto = async (e) => {
        ...
        if (files.length === 0 || !targetProjectId) return;

        setIsSubmitting(true);

        try {
            await Promise.all(
                files.map(file => db.processAndAddPhoto(file, targetProjectId))
            );
        } catch (err) {
            console.error("Error importing photos globally:", err);
        }

        await refreshProjects();
        setIsSubmitting(false);
        const pid = targetProjectId; // Keep ID for navigation
        ...
        // Navigate to the project gallery (main page) after import
        navigateTo('PROJECT_DETAIL', pid);
    };
    ...
    return (
        <div className="project-list-view" style={{ overflowX: 'hidden' }}>
            ...
            <div className="floating-dock">
                ...
                <button className="dock-btn" onClick={handleGlobalImportClick}><Upload size={22} /></button>
            </div>
            ...
            {/* Camera/Import Project Selector Modal */}
                ...
                <div className="modal-overlay" onClick={() => setShowCameraProjectSelector(false)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()}>
                        <h2 style={{ marginBottom: '1.5rem', fontSize: '1.3rem' }}>
                            {selectorMode === 'CAMERA' ? 'Camera: Select Project' : 'Import: Select Project'}
                        </h2>
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                if (selectorMode === 'CAMERA') {
                                                    ...
                                                    setTargetProjectId(p.ProjectID);
                                                    fileInputRef.current?.click();
                                                }
                                            }}
                                            style={{
